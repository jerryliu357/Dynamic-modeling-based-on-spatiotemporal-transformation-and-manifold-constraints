...
if __name__ == "__main__":
    ...
    M = 5  # 节点数
    st = time.time()
    record = generate_coupled_lorenz(N=M, L=4000, change1=2500, change2=2700)
    print(time.time()-st)
    save_results(record, 'record.npy')

    # 运行变化点检测（调整参数与MATLAB一致）
    result_x = temporal_cp_detection(
        record,
        maxstep=400,    # 与MATLAB的maxstep=400对齐
        L=4,             # 嵌入维度调整为4
        s=600,           # 随机组合数增加至600
        j=0,             # 目标变量索引（例如第一个节点）
        n_jobs=50
    ...
    result_y = temporal_cp_detection(
        record,
        maxstep=400,    # 与MATLAB的maxstep=400对齐
        L=4,             # 嵌入维度调整为4
        s=600,           # 随机组合数增加至600
    ...
    result_z = temporal_cp_detection(
        record,
        maxstep=400,    # 与MATLAB的maxstep=400对齐
        L=4,             # 嵌入维度调整为4
        s=600,           # 随机组合数增加至600
    ...
    # 保存结果与可视化（其余代码不变）
    save_results(result_x, 'detection_result_x.npy')
    save_results(result_y, 'detection_result_y.npy')
    save_results(result_z, 'detection_result_z.npy')
